...
> GM GRADE 9.5/10 - Solid code execution and generally good conceptual understanding. A few minor slips in terminology and interpretation (especially around precision/accuracy, bootstrap purpose, and confidence interval language) prevent a higher score.
...
> The variable le has the largest population standard deviation. This means latent heat flux varies the most over time compared to other variables.
---
...
> The variable fc had the largest relative error. This could be because carbon flux changes a lot over different time periods. Variables with high variability and a small sample may might not show all of the possibel values. More samples might be needed to see the full range of values.
...
summary_relerr <- sim_relerr %>%
  ...
  summarise(
    ...
    p90 = quantile(rel_error_pct, 0.9))
summary_relerr

p_relerr <- ggplot(summary_relerr, aes(x = n, y = median)) +
  geom_line() +
  labs(
...
> Relative error decreases as sample size increases. The chnage from 25 to 100 is larger than from 300 to 1000 because lower increases in sample size reduce uncertainty more at first. After that, adding more points offers diminishing returns because the wide range of sample valeus might have already been taken into account.
...
> The theoretical SE and simulated SE basically the same. Increasing 'n_samp' makes the sampling distribution narrower. Increasing 'reps' makes the simulation estimate more reliable.

> GM: "more reliable as it more closely resembles and behaves like the normal distribution"
...
> The bootstrap distribution is similar to the sampling distribution of the mean. The bootstrap is trying to approximate the true sampling distribution. It is useful when we only have one dataset and cannot repeatedly sample the population we are looking at.
...
> The bootstrap distribution is centered around the population mean. The bootstrap SD represents the estimated standard error of the mean. It shows how much the estimate could vary when sampling.
...
> The bootstrap distribution does look narrow even though its far from the population mean. Precision means the estimates are consistent. accuracy means they are close to the correct number. The summer sampling introduced bias from the time period or the seasons in our case.
...
> The random sample and stratified sample were close to the population mean. The summer sample was much lower. This shows that the summer sample is biased. The stratified sample was the most accurate because it included observations from every month. The bootstrap distributions for all of the samples were precise, but not all accurate. This shows that bootstraps can give us precision but not always accuracy or fixes for sampling bias.
---
...
> Yes, the confidence interval contains the true population mean. This shows that the the uncertainty is low in our estimate because we had tight values given our intervals.
---
...
> The red lines show the lower and upper bounds of the bootstrap confidence intervals.

> GM: -0.25 The red lines represent the 2.5th and 97.5th percentiles of the bootstrap distribution, forming the 95% bootstrap confidence interval for the mean.
...
> The slope interval didnt have zero. This means that there is a relationship between the predictor and response variable. The negatiev slope shows that as the predictor increases, the response variable decreases.

> GM: -0.25 Good observation that the interval does not include zero, implying a **significant** negative relationship. Best phrasing: “We can say with 95% confidence that the true slope lies between these bounds and is negative and significant.”
